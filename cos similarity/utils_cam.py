...
hands = mp_hands.Hands(max_num_hands=2, min_detection_confidence=0.5, min_tracking_confidence=0.5, model_complexity=1)  # 设置最大追踪手的数量
...
def procedure(video_path, crop, show = False):
    ...
    while cap.isOpened():
        ...
        cv2.imwrite(fr'output/original-{i}.jpg', frame)
        ...
        if show:
            ...
            cv2.imwrite(fr'output/background-{i}.jpg', frame)
...
def pre_adjust(frame, crop):
    if crop:
        ...
        frame = cv2.resize(frame, (int(width*480/height), 480))
        height, width, _ = frame.shape
        box_size = 480 
        x = (width - box_size) // 2
        y = (height - box_size) // 2
...
def visualize(input_file):
    ...
    for index, keypoints in enumerate(keypoint_coordinates):
        # 创建黑色背景的副本
        ...
        cv2.imwrite(fr'output/bright-{index}.jpg', frame)
...
def web_cam(break_time=2, save_folder = ''):
    segment = []
...
def node_json(input_video, output_folder, show):

    keypoint_coordinates,i = procedure(input_video, crop=True, show = show)
...
# def split_json(input_json, output_folder):
...
#     try:
#         keypoint_coordinates = fixed(coordinates_jason)
...
#         output_file = os.path.join(output_folder, f'segment'+'.json',)
