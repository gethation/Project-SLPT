...
def video_process_stream(model, dataset_dir, index, mean, std, augment, i=0):
    ...
    # visualize(json_path)
    x = load_json(json_path, mean, std, augment=augment).to('cuda')
    prediction = model(x)

def get_class_features(dataset_dir, output, mean, std, model, sample_num):
    class_features = {}
...
def get_experiment_features(dataset_dir, output, mean, std, model):
    class_features = {}
    sample_num  = 1
    for index in tqdm(range(len(list_folders(dataset_dir)))):
        try:
...
def integrate(mean, std, model, name, catagory):
